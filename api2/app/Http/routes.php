...
// Member Creation Routes
	...
	Route::post('/user',function(){
		...
		$permissions = array('user_id'=> $user->id,'administrator'=>"0",'dj'=> "1",'member'=> "0",'staff'=> "0",'volunteer'=> "0",'workstudy'=> "0");
	...
	Route::group(array('prefix'=>'{id}'),function($id = id){
		...
		Route::post('/',function($id){
			...
			foreach($existing_to_delete as $key=>$e){
				...
				if(!$still_exists) Showtime::find($e['id'])->delete(); //fimo: threw up errors... don't you love my vague bug report?
			}
			...
			foreach($showtimes as $item){
				if(isset($item['id'])){
					...
					$show_times[] = Showtime::create($item); //fimo: threw up errors... don't you love my vague bug report?
				}
			}
			return Response::json(array('show'=>$show,'social'=>$socials,'owners'=>$owners,'showtimes'=>$show_times));
		});
