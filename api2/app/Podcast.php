...
//	$response = 'MEEEOWWWWWW';
    ...
    private function make_audio(){
	    ...
	    $start_date =  date('d-m-Y+H%3\Ai%3\As', $start);
	    $end_date =  date('d-m-Y+H%3\Ai%3\As', $end);
		...
		$archive_access_url = "http://archiver.cjsf.ca/py/arch/download?";
    	...
    	$tags = array(
	        ...
	        'album'         => array('CJSF Radio Podcasts'),
	        'year'          => array($year),
	        'genre'         => array($this->playsheet->show->primary_genre_tags),
	        'comment'       => array('This podcast was created in part by CJSF Radio 90.1 FM')
    	);
    		$target_dir = $_SERVER['DOCUMENT_ROOT'].'podcasting/audio/'.$year.'/';
...
//    	$target_url = 'http://playlist.citr.ca/podcasting/audio/'.$year.'/'.$file_name;
    	$target_url = 'http://djland.cjsf.ca/podcasting/audio/'.$year.'/'.$file_name;

    	//Get Audio from Archiver
    	$file_from_archive = fopen($archive_url,'r');
	stream_set_blocking($file_from_archive, TRUE);
	stream_set_timeout($file_from_archive, 3600);
	...
	private function overwrite_audio(){
	    ...
	    $target_dir = $_SERVER['DOCUMENT_ROOT'].'podcasting/audio/';
	    if($this->url != null){
