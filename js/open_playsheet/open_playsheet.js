	...
	app.controller('openPlaysheetController',function(call,$window,$scope){
        ...
        this.log_error = function(error){
            ...
            call.error( this_.error).then(function(response){
                $('#error').append('Please contact CJSF Programming Coordinator at cjsfprog@sfu.ca . Your error has been logged');
            },function(error){
                $('#error').append('Please contact CJSF Programming Coordinator at cjsfprog@sfu.ca . Your error could not be logged :(');
            });
        }
...
//			console.log('playsheet[i] = '+i);
			var i = this_.playsheets.findIndex(function(object){
			return object.id == id;					
			});
			console.log(i);
//console.log('meow meow: '+this_.playsheets.filter(function(object){if(object.id == id) return this_.playsheets.indexOf(object);})[6]);
